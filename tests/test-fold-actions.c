...
create_view (void)
{
	...
	gtk_text_buffer_insert_at_cursor (buffer, "//test program\n\nvoid hello()\n{\n"
         "printf(\"hello World\");\n}\n\nint main(int argc char* argv)\n{\nhello();\n}\n", -1);

	gutter = gtk_source_view_get_gutter (GTK_SOURCE_VIEW (view), GTK_TEXT_WINDOW_LEFT);
	renderer = tepl_gutter_renderer_folds_action_new ();
	gtk_source_gutter_insert (gutter, renderer, 0);
	//gtk_text_buffer_get_iter_at_line (buffer, &start_iter, 2);
	//gtk_text_buffer_get_iter_at_line (buffer, &end_iter, 4);
